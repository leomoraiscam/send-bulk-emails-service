...
describe('Domain email entity', () => {
  it('should not be able to create a email when the same contains null strings', () => {
    expect(Email.create(null) as Email).toBe('InvalidEmailError');
  });
  ...
  it('should not create user with invalid email address', () => {
    expect(Email.create('invalid_email.com') as Email).toBe(
      'InvalidEmailError'
    );
  });
  ...
  it('should not accept local part larger than 64 chars', () => {
    expect(Email.create(`${'l'.repeat(65)}@email.com`)).toBe(
      'InvalidEmailError'
    );
  });
  ...
  it('should not accept empty local part', () => {
    expect(Email.create(`@email.com`)).toBe('InvalidEmailError');
  });

  it('should not accept empty domain part', () => {
    const email = `@`;

    expect(Email.create(email)).toBe('InvalidEmailError');
  });
